        ...
        if form.is_valid():
            ...
            try:
                user = CustomUser.objects.create_user(
                    email=email, password=password,hall_ticket_number='staff' ,section=section,user_type=2, first_name=first_name, last_name=last_name, profile_pic=passport_url)
                user.gender = gender
...
def add_student(request):
    ...
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            hall_ticket_number = student_form.cleaned_data.get('hall_ticket_number')
            ...
            try:
                ...
                user = CustomUser.objects.create_user(
                    email=email, password=password,hall_ticket_number = hall_ticket_number ,section=section_id,user_type=3,first_name=first_name, last_name=last_name, profile_pic=passport_url)
                user.gender = gender
                user.address = address
                user.student.session = session
                user.student.course = course
                user.student.section = section
                user.student.hall_ticket_number = hall_ticket_number
                user.save()
                messages.success(request, "Successfully Added")
...
def edit_student(request, student_id):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            hall_ticket_number = form.cleaned_data.get('hall_ticket_number')
            passport = request.FILES.get('profile_pic') or None
            try:
                ...
                user.hall_ticket_number = hall_ticket_number
                user.save()
                student.save()
                messages.success(request, "Successfully Updated")
    ...
    if request.method == 'POST':
        if form.is_valid():
            try:
                ...
                messages.error(
                    request, "Section Could Not Be Updated " + str(e))
...
def delete_section(request, section_id):
    ...
    try:
        ...
        messages.error(
            request, "Sorry, some students are assigned to this section already. Please move them to another section.")
